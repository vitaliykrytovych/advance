...
document.getElementById('x').style.display = 'none'; //початкова невидимість "очистки" input (хрестика)
        ...
        fetchListPromise()
            .then(function (a) {
                for (let i = 0; i < a.Search.length; i++) {
                    ...
                    <div class="details" onclick="detailsClickAlt(event)" data-id="${a.Search[i].imdbID}">More details</div>
                </div>`;
...
function fetchListPromise() {
    return new Promise(function (resolve, reject) {
        ...
        xhr.onerror = function (error) {
            reject('Error: No list of movies received from server!');
        };
        xhr.send();
    });
};
...
function detailsClickAlt(event) {
    fetchDetailsPromise()
        .then(function (a) {
            let elem =
                `<div class="background_container" onclick="backgroundClick(event)">
...
function fetchDetailsPromise() {
    return new Promise(function (resolve, reject) {
        ...
        xhr.onerror = function (error) {
            reject('Error: No data for the requested movie was received from the server!');
        };
        xhr.send();
    });
};
